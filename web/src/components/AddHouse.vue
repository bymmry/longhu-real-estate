      ...
      <el-form ref="form" label-width="200px" v-if="!loading">
        <el-form-item :label="item.label" v-for="item in form" :key="item.value">
          ...
          <div class="flex">
            ...
            <el-input-number style="flex:1" v-else-if="item.needSum" v-model="item[item.value]" :min="0" :max="1000000000000000" :label="'请输入或选择'+item.label"></el-input-number>
  ...
  export default {
    ...
    methods: {
      async add(data = {}) {
        ...
        let res = await this.$ajax.post('/house', data)
        loading.close()
        if (res) {
          this.$message({
            message: '成功录入一个房屋',
      ...
      async setForm() {
        ...
        for (let i = 0; i < field.length; i++) {
          ...
          // }
          // if (item.value === 'devCompanyName') {
          //   item.option = await this.getOption('/quick/devCompany')
          // }
          // if (item.value === 'karmaName') {
          //   item.option = await this.getOption('/quick/karma')
          // }
          // if (item.value === 'communityName') {
          //   item.option = await this.getOption('/quick/community')
          // }
          
          // if (item.value === 'houseCategoryName') {
          //   item.option = await this.getOption('/quick/houseCategory')
          // }
          // if (item.value === 'houseNatureName') {
          //   item.option = await this.getOption('/quick/houseNature')
          // }
          if (item.needSum) {
            item[item.value] = 0
          }
